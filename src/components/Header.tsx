    ...
    <>
      <header
        className={cn(
          ...
          showSolid
            ? "bg-white/95 backdrop-blur-sm border-b border-[#e3e3e3]"
            : "bg-transparent"
        )}
      >
        <div className="flex items-center lg:items-start justify-between px-6 lg:px-10 pt-4 lg:pt-7 pb-4">
          <Link
            href={homeHref}
            className={cn(
          ...
          >
            <span className="text-[15px] lg:text-2xl tracking-[0.08em] lg:tracking-[0.15em] text-[#222] whitespace-nowrap">
              ささき矯正歯科クリニック
            </span>
            <span className="hidden lg:block text-[11px] tracking-[0.25em] mt-1 text-[#9a998e]">
          ...
          <button
            ...
            aria-label={open ? "メニューを閉じる" : "メニューを開く"}
            aria-expanded={open}
            className="lg:hidden relative h-[14px] w-7 flex flex-col justify-between focus:outline-none"
          >
            <span className={cn("block h-[1.5px] w-full transition-all duration-300", showSolid ? "bg-[#222]" : "bg-white", open && "translate-y-[6px] rotate-45")} />
            <span className={cn("block h-[1.5px] w-full transition-opacity duration-200", showSolid ? "bg-[#222]" : "bg-white", open && "opacity-0")} />
            <span className={cn("block h-[1.5px] w-full transition-all duration-300", showSolid ? "bg-[#222]" : "bg-white", open && "-translate-y-[6px] -rotate-45")} />
